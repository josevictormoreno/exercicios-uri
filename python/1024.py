...
while i < casos:
    string = input()
    for letra in string:
        if (ord(letra) > 64 and ord(letra) < 91) or (ord(letra) > 97 and ord(letra) < 122):
            criptoUm += chr(ord(letra) + 3)
        else:
            criptoUm += letra
    
    criptoDois = inverter(criptoUm)
    criptoTres = criptoDois[:len(criptoDois)//2]
    aux = criptoDois[len(criptoDois)//2:]

    for letra in aux:
        criptoTres += chr(ord(letra) - 1)
